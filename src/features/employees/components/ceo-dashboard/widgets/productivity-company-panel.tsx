...
function useChartBoxSize(fallback = 160) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const next = Math.floor(Math.min(width, height, 220));
...
export const ProductivityCompanyPanel: React.FC = () => {
  const { summary, delta, status } = ceoDashboardData.productivity;
  ...
  const { ref: gaugeRef, size: chartSize } = useChartBoxSize(160);
