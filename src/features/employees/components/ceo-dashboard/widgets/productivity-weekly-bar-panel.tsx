...
export const ProductivityWeeklyBarPanel: React.FC = () => {
  ...
  const { containerRef, height } = usePanelChartHeight(190);
  const chartHeight = Math.max(height, 175);
  ...
  const options: ApexOptions = useMemo(
    () => ({
      ...
      dataLabels: {
        ...
        offsetY: -18,
        style: { fontSize: '13px', fontWeight: 700, colors: [chartTokens.textColor] }
      },
      colors: [chartTokens.primary],
      grid: {
        borderColor: chartTokens.gridColor,
        strokeDashArray: 4,
        padding: { top: 24, right: 10, left: 6, bottom: 4 },
  ...
  return (
    <article className="eto-widget cpr-panel cpr-cell--weekly cpr-trend">
      ...
      <footer className="cpr-trend__footer">
        ...
        <span className="cpr-trend__stat cpr-trend__stat--peak">
