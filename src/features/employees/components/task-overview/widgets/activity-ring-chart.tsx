...
interface ActivityRingChartProps {
  ...
  /** Fixed px size — skips resize observer to avoid chart growth loops. */
  fixedSize?: number;
}
...
export const ActivityRingChart: React.FC<ActivityRingChartProps> = ({
  ...
  const chartSize = fixedSize ?? Math.max(148, height);
  ...
  return (
    <>
      <div
        ref={fixedSize ? undefined : containerRef}
        className="eto-pie__chart-body"
        style={
          fixedSize
            ? { width: fixedSize, height: fixedSize, flexShrink: 0 }
            : undefined
        }
      >
        <Chart
